...
public class GetProductArticles {
  private final ReadArticle articleReader;
  private final PayingHandler payingHandler;

  public GetProductArticles(PayingHandler payingHandler, ReadArticle articleReader) {
    this.payingHandler = payingHandler;
    this.articleReader = articleReader;
  }

  public final Either<ProtostellarError, List<Article>> perform(Identifier userId, Identifier productId) {
    return payingHandler.hasProperPayingAccessFor(userId, PayingFeature.MAINTENANCE_FEATURE)
      .flatMap(v -> articleReader.getProductArticles(productId));
  }
}
